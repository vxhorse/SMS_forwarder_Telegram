...
你能看到的是容器在 `healthy` 与 `unhealthy` 之间反复跳动：快照只在全部组件就绪时才写，
所以在每一段掉线期间它的 mtime 都不再推进，而这期间到达的短信不会被投递。这本身不会
触发任何重启——健康检查失败并不会让容器重启。若怀疑是这种情况，请把快照里的
`reconnects` 计数与容器健康状态的跳变放在一起看。
