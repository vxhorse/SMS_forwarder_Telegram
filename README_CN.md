...
| `SMS_BAUDRATE` | `115200` | 串口波特率 | 无。与模块不匹配的速率表现为「所有候选端口都不应答 `AT`」，日志为 `No candidate port answered AT`，其中不含端口名——逐个端口的那几行都在 DEBUG 级。设 `LOG_LEVEL=DEBUG` 才能看到试过哪些、哪些没应答 |
...
| `WATCHDOG_DOWN_SECONDS` | `3600` | 组件掉线超过该时长后退出进程 | 故意不设上限。它自身就是 `WATCHDOG_STALL_SECONDS` 的上限；若被设到该设置推导下限之下，以下限为准，并由启动通知说明该不变量对这份配置已不成立。实际上它有一条下界：必须高于 `SERVICE_STABLE_SECONDS` 加 `WATCHDOG_CHECK_INTERVAL` 再加你的模块连接所需的时间，否则看门狗会在任何组件到达首次恢复之前就退出——在完好的硬件上形成永久重启循环。启动通知会报出这份配置里能算出的那部分，连接耗时需要你自己加上 |
| `WATCHDOG_STALL_SECONDS` | *(推导得出)* | 组件循环虽仍报告就绪却停止推进达该时长后退出进程 | 默认取 `HEALTH_STALE_SECONDS` 的两倍，再对它施加一条推导出的下限（**当前默认为 310**），并以 `WATCHDOG_DOWN_SECONDS` 为上限，下限最后生效。低于该下限，看门狗会重启一个只是在耐心等待慢模块的进程 |
...
推得最狠的是 `MODEM_REGISTRATION_CHECK=1`，它把下限推到 3600，在所有设置里推得最远：
该检查要连续读到 `MODEM_REGISTRATION_FAILURES` 次才结束会话，而不是一次，
因此它是把最慢周期成倍放大，而不是加上一点。
但它并不是唯一一个在其余设置都保持默认时就越过 1800 的。
`WATCHDOG_CHURN_SESSIONS=13` 把下限推到 1820，`=20` 推到 2800，
两者都不会有任何启动通知，因为这条下限就是这个次数乘以一个最坏周期。
`RECONNECT_BACKOFF_MAX=90` 会把下限推到 2000，且文件里没有别的东西会对它有反应。
放慢探测计划也是一样——`MODEM_PROBE_INTERVAL=50` 推到 1840，`=60` 推到 2140，
`MODEM_PROBE_FAILURES=5` 推到 1860——不过这三种配置整体上并不安静：
它们同时会把 `MODEM_PROBE_TIMEOUT` 的上限压垮，
...
因此每个周期都会把掉线计时与停滞计时清零；但快照不随之刷新——
它只在全部组件就绪时才写，所以从失败那一刻到下一次恢复，它的 mtime 都不再推进。
这段间隔在拆链、退避与重连之外必然还含有 `SERVICE_STABLE_SECONDS`，
吃掉的是 `HEALTH_STALE_SECONDS` 的绝大部分而不是其中一瞬，
所以容器健康检查在两个周期之间只是勉强保持绿色。重连频次判据确实会把这些周期计入，
...
因此无论开关与否，状态都会被解析、写入快照，并在 `LOG_LEVEL=DEBUG` 下记入日志；
被推迟的只是「是否因此结束会话」这一动作。请在包含正常短信往来的一段时间内观察
`registration` 字段：
...
- 一直是 `null`——说明模块自上次初始化以来没有主动上报过驻网变化：关闭该检查时
  本服务不会主动发问 `AT+CREG?`，这个字段只由模块自发的 `+CREG` 上报填充。
...
- **`unhealthy`**——除下文「快照写不出去」那一种情形外，此时没有任何短信被转发。
  ...
  看门狗有三条退出路径，各自问的问题不同——这个组件此刻处于什么状态、
  ...
  - **反复失败又反复恢复**的组件，从构造上就对上面两条不可见。
    ...
    会从上一次恢复起继续量它。尚未插入的模块根本到不了会话这一步，
    因此这里永远不会计入它；量它的是掉线计时——因为没有可供起算的恢复，
    就从进程启动起算——进程会在 `WATCHDOG_DOWN_SECONDS` 时退出，
    再被重启回同一段等待。
- **`healthy`**——快照是新鲜的，且全部组件均已就绪。

有一种形态落在这三条判据之间，这里如实写下，而不是声称已经防住。掉线计时是从该组件
**上一次恢复**起算的，而不是从进程启动起算，所以一个交替出现的组件——先在
`SERVICE_STABLE_SECONDS` 之内连续失败若干次，再有一次会话撑过了这个窗口——会用那一次
恢复把计时清零，同时只给重连计数加一。两个读数都到不了各自的阈值，进程会继续跑下去。

你能看到什么，取决于每一段掉线持续多久。快照只在全部组件就绪时才写，所以从失败那一刻
到下一次恢复，它的 mtime 都不再推进——这段间隔必然包含 `SERVICE_STABLE_SECONDS`，
因为在那之前不会有组件被标记为就绪。间隔超过 `HEALTH_STALE_SECONDS` 之后，容器才会变成
`unhealthy` 并反复跳动；周期短到始终留在这个窗口之内的，则根本不会触发它，此时快照里的
`reconnects` 计数是唯一还在变化的读数。这两种情况本身都不会触发重启——健康检查失败
并不会让容器重启。若怀疑是这种形态，请把这些计数与容器健康状态的跳变放在一起看。

两种情况里只有一种会丢短信。模块组件掉线期间，到达的短信由模块自己存起来，下一次
初始化会把它们读出来，并且只有在能够解码的条目全部投递出去时才清空存储区，
所以只是延迟送达而不是丢失。Telegram 组件掉线期间，模块是正常的，
`AT+CNMI=2,2,0,0,0` 会把每条短信直接交给本进程，而此时它无处转发。
...
- **稳定性**：各组件独立按指数退避重连；看门狗在三种信号中的任意一种下让进程退出、再由容器重启策略接手，而健康快照写不出去是唯一一种只报出、不退出的故障。三种信号分别是什么、各自以哪条阈值衡量，详见「读懂健康检查」一节
- **串口选择**：优先让 `SMS_PORT` 保持为空，由自动发现决定；只有当发现选错设备时才设置，并填写 `SMS_DEV_ROOT` 之下的路径
- **没有硬件不算错误**：未接入模块时，服务会无限期等待并重试，既无启动期限也无重试次数上限，其间容器报告为不健康。但进程本身并非永久：从未连上过的组件从进程启动起就算作掉线，因此看门狗会在 `WATCHDOG_DOWN_SECONDS`（默认一小时）时结束进程，由重启策略重新开始同一段等待。没插模块的机器上每小时加一次重启计数是这个，不是崩溃
...
4. **本项目尚未验证过的模块**：
   ...
   - 只要补读出的、能够解码的条目没有全部投递出去，擦除就是被跳过而不是被容忍——
     Telegram 一侧还在连接时这是常态。日志为
