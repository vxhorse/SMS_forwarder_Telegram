...
**镜像标签就是发布日期**，`YYYY-MM-DD`——例如 `2026-08-06`，同一天的第二次发布是
`2026-08-06b`。它们不是版本号：本项目不承诺各次发布之间的兼容性，而日期能回答
你对一个跑了一阵子的容器真正关心的问题——它有多旧。`latest` 始终指向最新的那次发布。
想自己决定何时升级，就固定到某个日期。

`1.0` 到 `2.0` 是这套规则之前的标签，已经冻结、仅 `amd64`，且早于本文所述的可靠性改造——
**不要把数字最大的当成最新镜像。**
