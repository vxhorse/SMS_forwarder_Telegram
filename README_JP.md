...
| `MODEM_REGISTRATION_FAILURES` | `3` | 上記を有効にした場合、再接続を強制する「圏外」の連続読み取り回数 | 下限2：ハンドオーバー中は在圏状態が一瞬途切れるため、1回の読み取りは何の証拠にもなりません。またゼロは設定のように見えながら確認そのものを切ってしまいます。上限は意図的になし——辛抱強くなるほどセッション終了が遅れるだけで、確認が有効な間は `WATCHDOG_CHURN_WINDOW` の下限がこの回数で乗算されるため、窓はそれだけ遅くなった周期に追いつかなくなることはありません |
