...
**イメージタグはリリース日**です。`YYYY-MM-DD`——たとえば `2026-08-06`、同じ日の
2回目のリリースは `2026-08-06b` です。バージョン番号ではありません。リリース間の
互換性は約束していませんし、しばらく動いているコンテナについて実際に知りたいのは
「どれくらい古いか」であり、日付はそれに答えます。`latest` は常に最新のリリースを
指します。更新の時期を自分で決めたい場合は日付で固定してください。

`1.0` から `2.0` はこの方式より前のタグです。凍結済みで `amd64` のみ、本書が説明する
信頼性の改善よりも古いものです——**数字が最大のものを最新イメージだと読まないで
ください。**
...
コンテナは最大で約4分30秒のあいだ `starting` を報告しますが、これは正常です。
...
- **`starting`**——`start_period` の範囲内です。サンプルの構成ファイルでは180秒です。
  ...
  待ち時間は `start_period` だけではありません。その内側で失敗した検査は数えられないため、
  続いているあいだ状態は変わりません。過ぎたあとも、検査が `interval` 間隔で `retries` 回
  連続して失敗するまで——サンプルファイルでは30秒×3回——コンテナは `unhealthy` に
  なりません。上で述べた4分30秒はこの合計です。決して起動しないコンテナが数分のあいだ
  起動しそうに見えるのは、このためです。
...
5. **コンテナがいつまでも healthy にならない**：
   - 最大で約4分30秒の `starting` は正常です。「ヘルスチェックの読み方」の節を参照してください
