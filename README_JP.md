...
なります。他が既定のままで1800を超える唯一の設定でもあります——この確認は1回ではなく
`MODEM_REGISTRATION_FAILURES` 回の読み取りを経てセッションを終わらせるため、最も遅い周期に
足すのではなく掛けるからです。`RECONNECT_BACKOFF_MAX=90` で下限は2000になり、ファイル内の他の
...
- **`unhealthy`**——SMSは一切転送されていません。これ自体は何も再起動しないことに
  ...
  - **失敗と復旧を繰り返す**コンポーネントは、構造上、上の2つからは見えません。
    ...
    モジュールはセッションに到達すらしないので、これまでどおり無期限に待たれます。
...
届いたSMSはモジュールが保存し、次の初期化が消去の前に読み出すので、失われるのではなく
遅れて配送されます。Telegram側のコンポーネントが停止している間はモジュールが稼働中なので、
...
- **安定性**：各コンポーネントは指数バックオフで自律的に再接続し、ウォッチドッグは3つの信号のいずれかでプロセスを再起動します。ヘルススナップショットが書けない場合だけは、再起動せずに報告のみ行います。3つが何で、それぞれどのしきい値で測られるかは「ヘルスチェックの読み方」の節を参照してください
