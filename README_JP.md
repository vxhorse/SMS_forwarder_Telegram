...
ます。代わりに見えるのは、コンテナが `healthy` と `unhealthy` のあいだで揺れることです。
スナップショットは全コンポーネントが稼働中のときだけ書かれるため、停止している区間では
mtime が進まず、その間に届いたSMSは配送されません。これ自体は何も再起動しません——
ヘルスチェックの失敗で再起動は起きないからです。疑わしいときは、スナップショットの
`reconnects` の値をコンテナの健全性の遷移と並べて見てください。
