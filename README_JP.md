...
クランプされ、境界が動かした設定は起動ログがすべて名指しします。上限を持たない設定は
「持たない」ことと「なぜ持たないか」を明記するので、境界が無いことを
書き忘れと読み違えることはありません。
...
このうち2つの境界は固定ではなく導出値なので、元になる設定が動けば一緒に動きます。
とくに注意すべきは `WATCHDOG_CHURN_WINDOW` の下限です。`WATCHDOG_CHURN_SESSIONS`、
`RECONNECT_BACKOFF_MAX`、そしてモジュール監視のスケジュールから導かれるため、
既定値の1800を超えるまでの余裕は数字の見た目ほどありません。`MODEM_PROBE_INTERVAL=50`
——それ自体は許容範囲に十分収まっています——で下限は1840、`=60` では2140になります。
実際に効いている値は下限が勝つので安全ですし、`WATCHDOG_CHURN_WINDOW` を自分で設定した人
——`.env.example` をコピーした人はすべて該当します——には上書きを名指しする起動通知が出ます。
静かなのは `WATCHDOG_CHURN_WINDOW` を設定しなかった場合だけで、そのとき引き上げられているのは
運用者が求めた値ではなく既定値です。
...
- **`unhealthy`**——SMSは一切転送されていません。これ自体は何も再起動しないことに
  ...
  引き継ぎます。ウォッチドッグの終了経路は3つあります。互いに重なりません。
  それぞれが別の問いを立てているからです——このコンポーネントは今どの状態か、
  そのループはまだ動いているか、そして最近その状態に何回入ったか：
