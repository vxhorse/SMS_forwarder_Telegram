...
| `SERVICE_STABLE_SECONDS` | `60.0` | 復旧とみなすまでのセッション継続時間 | 下限5：ゼロでは接続した瞬間に復旧とみなされ、それこそがこの設定が防いでいることです。上限はありませんが、無償でもありません：セッションがこの時間続くまで稼働中とは記録されないため、これと `WATCHDOG_CHECK_INTERVAL` の合計は `WATCHDOG_DOWN_SECONDS` を下回っている必要があり、下回らない場合は起動通知が知らせます |
